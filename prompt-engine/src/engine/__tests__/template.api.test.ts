...
describe('Prompt Template API', () => {
	...
	describe('GET /template/all', () => {
		it('should return all templates', async () => {
			getAllPromptTemplates.mockResolvedValue([{ id: '1' }, { id: '2' }])
		...
		it('should handle errors', async () => {
			getAllPromptTemplates.mockRejectedValue(new Error('fail'))
			const res = await request(app).get('/template/all')
	...
	describe('GET /template/:id', () => {
		it('should return template by id', async () => {
			getPromptTemplateById.mockResolvedValue({ id: 'abc' })
		...
		it('should return 404 if not found', async () => {
			getPromptTemplateById.mockResolvedValue(null)
		...
		it('should handle errors', async () => {
			getPromptTemplateById.mockRejectedValue(
				new Error('fail')
			)
			const res = await request(app).get('/template/abc')
			expect(res.status).toBe(500)
	...
	describe('DELETE /template/:id', () => {
		it('should delete template by id', async () => {
			deletePromptTemplate.mockResolvedValue({ id: 'abc' })
			const res = await request(app).delete('/template/abc')
			expect(res.status).toBe(200)
			expect(res.body.id).toBe('abc')
		})

		it('should return 404 if not found', async () => {
			deletePromptTemplate.mockResolvedValue(null)
			const res = await request(app).delete('/template/xyz')
			expect(res.status).toBe(404)
			expect(res.body.error).toBe('Template not found')
		})

		it('should handle errors', async () => {
			deletePromptTemplate.mockRejectedValue(new Error('fail'))
			const res = await request(app).delete('/template/abc')
			expect(res.status).toBe(500)
